...
if (isset($_GET['delete_id'])) {
    ...
    $check_stmt = $conn->prepare("SELECT id, operator_name, month, bill_id FROM salaries WHERE id = ? AND company_id = ?");
    ...
    if ($salary) {
        ...
        try {
            ...
            if ($salary['bill_id'] && $salary['bill_id'] > 0) {
                $del_bill = $conn->prepare("DELETE FROM stitching_posted_bills WHERE id = ? AND company_id = ?");
                $del_bill->bind_param("ii", $salary['bill_id'], $company_id);
                $del_bill->execute();
            }
...
if (isset($_GET['post_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE salaries SET status = 'posted' WHERE id = ? AND company_id = ?");
    $stmt->bind_param("ii", $post_id, $company_id);
    $stmt->execute();
    header("Location: salary_list.php");
    exit();
}
if (isset($_GET['unpost_id'])) {
    $unpost_id = intval($_GET['unpost_id']);
    $stmt = $conn->prepare("UPDATE salaries SET status = 'un_posted' WHERE id = ? AND company_id = ?");
    $stmt->bind_param("ii", $unpost_id, $company_id);
    $stmt->execute();
    header("Location: salary_list.php");
    exit();
}
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="action-buttons">
                                    <a href="operator_salary.php?view_salary=<?= $row['id'] ?>" class="btn btn-info btn-sm" title="View Details">
                                        <i class="fas fa-eye"></i> View
                                    </a>
                                    
                                    <a href="edit_salary.php?id=<?= $row['id'] ?>" class="btn btn-primary btn-sm" title="Edit">
                                        <i class="fas fa-edit"></i> Edit
                                    </a>
                                   
                                   
                                    <a href="salary_list.php?delete_id=<?= $row['id'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Delete this salary record?')" title="Delete">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="12" class="empty-state">
                                ...
                                <a href="operator_salary.php" class="btn btn-primary btn-sm mt-3">Create New Salary</a>
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
