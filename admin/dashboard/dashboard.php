...
// Embroidery Vendors (for multi-select)
$embroidery_vendors = []; 
if ($embroidery_vendor_column) {
    if ($embroidery_vendor_column=='embroidery_vendor_id') {
        ...
        while($row=$res->fetch_assoc()) $embroidery_vendors[] = ['id'=>$row['vid'], 'name'=>$row['party_name']];
    } else {
        ...
        while($row=$res->fetch_assoc()) $embroidery_vendors[] = ['id'=>$row['vname'], 'name'=>$row['vname']];
    }
}

// ----- URL filters (multi-value support) -----
$selected_fabrics = isset($_GET['fabric']) ? explode(',', $_GET['fabric']) : [];
$selected_brands = isset($_GET['brand']) ? explode(',', $_GET['brand']) : [];
$selected_cmt_parties = isset($_GET['cmt_party']) ? explode(',', $_GET['cmt_party']) : [];
$selected_emb_vendors = isset($_GET['embroidery_vendor']) ? explode(',', $_GET['embroidery_vendor']) : [];
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filter-control { width: 100%; padding: 8px 12px; border-radius: 40px; border: 1px solid #e9ecef; background: white; font-size: 0.8rem; cursor: pointer; }
        .dropdown-menu-custom { max-height: 250px; overflow-y: auto; padding: 8px; }
...
<div class="main-container">
    ...
    <!-- Filter Bar with Multi-Select for all -->
    <div class="filter-bar">
        ...
        <div class="filter-group">
            ...
            <div class="dropdown">
                <button class="filter-control dropdown-toggle" type="button" data-bs-toggle="dropdown" style="text-align:left;">
                    <?= !empty($selected_brands) ? count($selected_brands).' selected' : 'All Brands' ?>
                </button>
                <ul class="dropdown-menu dropdown-menu-custom p-2">
                    <?php foreach($brands as $b): $chk = in_array($b, $selected_brands) ? 'checked' : ''; ?>
                    <li><div class="form-check"><input class="form-check-input brand-checkbox" type="checkbox" value="<?= htmlspecialchars($b) ?>" <?= $chk ?>> <label class="form-check-label"><?= htmlspecialchars($b) ?></label></div></li>
                    <?php endforeach; ?>
                </ul>
            </div>
        </div>
        ...
        <!-- CMT Party (multi-select) – shown only when applicable -->
        <?php if ($cmt_party_column && !empty($cmt_parties) && ($status_filter=='CMT' || $all_jobs_filter=='1')): ?>
        <div class="filter-group">
            <label>CMT Party</label>
            <div class="dropdown">
                <button class="filter-control dropdown-toggle" type="button" data-bs-toggle="dropdown" style="text-align:left;">
                    <?= !empty($selected_cmt_parties) ? count($selected_cmt_parties).' selected' : 'All Parties' ?>
                </button>
                <ul class="dropdown-menu dropdown-menu-custom p-2">
                    <?php foreach($cmt_parties as $p): $chk = in_array($p, $selected_cmt_parties) ? 'checked' : ''; ?>
                    <li><div class="form-check"><input class="form-check-input cmt-checkbox" type="checkbox" value="<?= htmlspecialchars($p) ?>" <?= $chk ?>> <label class="form-check-label"><?= htmlspecialchars($p) ?></label></div></li>
                    <?php endforeach; ?>
                </ul>
            </div>
        </div>
        ...
        <!-- Embroidery Vendor (multi-select) – shown only when applicable -->
        <?php if (!empty($embroidery_vendors) && ($status_filter=='Embroidery' || $all_jobs_filter=='1')): ?>
        <div class="filter-group">
            <label>Embroidery Vendor</label>
            <div class="dropdown">
                <button class="filter-control dropdown-toggle" type="button" data-bs-toggle="dropdown" style="text-align:left;">
                    <?= !empty($selected_emb_vendors) ? count($selected_emb_vendors).' selected' : 'All Vendors' ?>
                </button>
                <ul class="dropdown-menu dropdown-menu-custom p-2">
                    <?php foreach($embroidery_vendors as $v): $val = $v['id']; $name = $v['name']; $chk = in_array($val, $selected_emb_vendors) ? 'checked' : ''; ?>
                    <li><div class="form-check"><input class="form-check-input vendor-checkbox" type="checkbox" value="<?= htmlspecialchars($val) ?>" <?= $chk ?>> <label class="form-check-label"><?= htmlspecialchars($name) ?></label></div></li>
                    <?php endforeach; ?>
                </ul>
            </div>
        </div>
    ...
    <?php
    // Build main job query with multi-value filters
    ...
    if (!empty($selected_brands)) {
        $esc = array_map(function($b) use($conn){ return mysqli_real_escape_string($conn,$b); }, $selected_brands);
        $where[] = "brand_name IN ('".implode("','",$esc)."')";
    }
    
    // CMT Party filter (multi)
    if ($cmt_party_column && !empty($selected_cmt_parties)) {
        $esc = array_map(function($p) use($conn){ return mysqli_real_escape_string($conn,$p); }, $selected_cmt_parties);
        $where[] = "$cmt_party_column IN ('".implode("','",$esc)."')";
    }
    
    // Embroidery Vendor filter (multi)
    if ($embroidery_vendor_column && !empty($selected_emb_vendors)) {
        $esc = array_map(function($v) use($conn){ return mysqli_real_escape_string($conn,$v); }, $selected_emb_vendors);
        $where[] = "$embroidery_vendor_column IN ('".implode("','",$esc)."')";
    }
    ...
    if (!empty($selected_brands)) $title .= " (Brand: ".implode(', ',$selected_brands).")";
    ?>

    <div class="section-header">
        <h3><i class="fas fa-list-ul"></i> <?= $title ?> <span class="text-muted small">(<?= $jobs_result ? $jobs_result->num_rows : 0 ?>)</span></h3>
        <?php if($status_filter || $all_jobs_filter || !empty($selected_fabrics) || !empty($selected_brands) || !empty($selected_cmt_parties) || !empty($selected_emb_vendors)): ?>
...
function applyFilters() {
    ...
    // Brand (multi-select)
    let brands = [];
    $('.brand-checkbox:checked').each(function(){ brands.push($(this).val()); });
    brands.length ? url.set('brand', brands.join(',')) : url.delete('brand');

    // CMT Party (multi-select)
    let cmtParties = [];
    $('.cmt-checkbox:checked').each(function(){ cmtParties.push($(this).val()); });
    cmtParties.length ? url.set('cmt_party', cmtParties.join(',')) : url.delete('cmt_party');

    // Embroidery Vendor (multi-select)
    let vendors = [];
    $('.vendor-checkbox:checked').each(function(){ vendors.push($(this).val()); });
    vendors.length ? url.set('embroidery_vendor', vendors.join(',')) : url.delete('embroidery_vendor');
