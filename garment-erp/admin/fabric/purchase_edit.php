<?php
$page_identifier = 'fabric/purchase_edit.php';
session_start();
...
// ---------- ensure required columns (same as add page) ----------
...
$fetch_stmt = $conn->prepare("SELECT * FROM fabric_purchase WHERE id = ? AND company_id = ?");
$fetch_stmt->bind_param("ii", $purchase_id, $company_id);
$fetch_stmt->execute();
$purchase = $fetch_stmt->get_result()->fetch_assoc();
if (!$purchase) {
    die("Purchase record not found.");
}

// Store old values for possible update of stitching_posted_bills
$old_party = $purchase['party_name'];
$old_fabric = $purchase['fabric_name'];
$old_meter = $purchase['total_meter'];
$old_rate = $purchase['rate'];
...
if (isset($_POST['save'])) {
    ...
    $created_at = $_POST['created_at'] ?? $old_created_at;

    // Verify party exists and belongs to company
    $party_check = $conn->prepare("SELECT id FROM parties WHERE party_name = ? AND company_id = ?");
    $party_check->bind_param("si", $party, $company_id);
    $party_check->execute();
    $party_exists = $party_check->get_result()->num_rows > 0;

    if (!$party_exists) {
        $error = "Party '$party' does not exist! Please add this party first in the Parties section.";
    } else {
        $total_amount = $meter * $rate;
        $description = "Fabric Purchase - Party: $party, Fabric: $fabric, Lot: $lot, Meter: $meter";

        mysqli_begin_transaction($conn);
        try {
            // 1. Update fabric_purchase (keep user_id unchanged)
            ...
            $update_purchase->bind_param("ssssssssddssii",
                $party, $fabric, $color, $lot, $bill, $challan, $bundle, $built,
                $meter, $rate, $adjust, $created_at,
                $purchase_id, $company_id);
            if (!$update_purchase->execute()) {
                throw new Exception("Error updating purchase: " . $update_purchase->error);
            }

            // 2. Update corresponding stitching_posted_bills record
            ...
            $update_bill = $conn->prepare("UPDATE stitching_posted_bills SET
                ...
                claim_date = ?,
                fabric_name = ?,
                user_id = ?          -- update to current editor (optional, remove if you want to keep original)
                WHERE job_no = ? AND company_id = ?");
            $update_bill->bind_param("sdddsssisi",
                $party,        // emp_name
                $meter,        // qty
                $rate,         // rate
                ...
                $fabric,       // fabric_name
                $user_id,      // user_id (updating editor)
                $job_no,       // job_no
                $company_id    // company_id
            );
            if (!$update_bill->execute()) {
                throw new Exception("Error updating stitching bill: " . $update_bill->error);
            }

            mysqli_commit($conn);
            header("Location: purchase_list.php?success=2");
            exit();
        } catch (Exception $e) {
...
// ---------- prepopulate form (use POST values if error, else DB values) ----------
$party      = isset($_POST['party_name']) ? $_POST['party_name'] : $purchase['party_name'];
$fabric     = isset($_POST['fabric_name']) ? $_POST['fabric_name'] : $purchase['fabric_name'];
$color      = isset($_POST['color']) ? $_POST['color'] : $purchase['color'];
$lot        = isset($_POST['lot_no']) ? $_POST['lot_no'] : $purchase['lot_no'];
$bill       = isset($_POST['bill_no']) ? $_POST['bill_no'] : $purchase['bill_no'];
$challan    = isset($_POST['challan_no']) ? $_POST['challan_no'] : $purchase['challan_no'];
$bundle     = isset($_POST['bundle_no']) ? $_POST['bundle_no'] : $purchase['bundle_no'];
$built      = isset($_POST['built_num']) ? $_POST['built_num'] : $purchase['built_num'];
$meter      = isset($_POST['total_meter']) ? $_POST['total_meter'] : $purchase['total_meter'];
$rate       = isset($_POST['rate']) ? $_POST['rate'] : $purchase['rate'];
$adjust     = isset($_POST['adjust_rate']) ? $_POST['adjust_rate'] : $purchase['adjust_rate'];
$created_at = isset($_POST['created_at']) ? $_POST['created_at'] : $purchase['created_at'];
?>
<!DOCTYPE html>
<html>
<head>
<title>Edit Fabric Purchase</title>
                    ...
                    <div class="form-column">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" name="lot_no" class="form-control" value="<?= htmlspecialchars($lot) ?>" required style="font-weight: 600; background: var(--primary-light);">
                                <small class="text-muted">Current Lot No.</small>
                            </div>
                            <div class="form-group">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="text" name="party_name" id="party_name" class="form-control" value="<?= htmlspecialchars($party) ?>" list="partyList" placeholder="Party name" required autocomplete="off">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="fabric_name" id="fabric_name" class="form-control" value="<?= htmlspecialchars($fabric) ?>" list="fabricList" placeholder="Fabric name" required autocomplete="off">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="color" class="form-control" value="<?= htmlspecialchars($color) ?>" placeholder="Color">
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="number" name="bill_no" class="form-control" value="<?= htmlspecialchars($bill) ?>" placeholder="Bill number">
                            </div>
                            <div class="form-group">
                                <label><i class="fas fa-truck"></i> Challan</label>
                                <input type="number" name="challan_no" class="form-control" value="<?= htmlspecialchars($challan) ?>" placeholder="Challan number">
                            </div>
                            <div class="form-group">
                                <label><i class="fas fa-cubes"></i> Bundle</label>
                                <input type="number" name="bundle_no" class="form-control" value="<?= htmlspecialchars($bundle) ?>" placeholder="Bundle number">
                            </div>
                            <div class="form-group">
                                <label><i class="fas fa-cog"></i> Built</label>
                                <input type="text" name="built_num" class="form-control" value="<?= htmlspecialchars($built) ?>" placeholder="Built number">
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <input type="number" step="0.01" name="total_meter" id="total_meter" class="form-control" value="<?= htmlspecialchars($meter) ?>" placeholder="0.00" required oninput="calculateTotals()">
                            </div>
                            <div class="form-group">
                                <label class="required"><i class="fas fa-tag"></i> Rate</label>
                                <input type="number" step="0.01" name="rate" id="rate" class="form-control" value="<?= htmlspecialchars($rate) ?>" placeholder="0.00" required oninput="calculateTotals()">
                            </div>
                            <div class="form-group">
                                <label><i class="fas fa-adjust"></i> Adjustment (Info Only)</label>
                                <input type="number" step="1" name="adjust_rate" id="adjust_rate" class="form-control" value="<?= htmlspecialchars($adjust) ?>" placeholder="0.00" oninput="calculateTotals()">
...
function calculateTotals() {
    ...
    const finalAmount = subTotal;  // same as add page logic
...
document.getElementById('party_name').addEventListener('input', checkPartyExists);
document.getElementById('party_name').addEventListener('blur', checkPartyExists);
